...
- `run_animation(mesh, rotate, frame_limit, exposure, use_flow)`：运行帧循环。
- `main`：解析 `--sphere`、`--hitchcock`、`--camera-auto`、`--long-exposure`、`--flow-exposure`、`--export-image`、`--show-image`、`--record`、`--play`、`--duration`、`--fps` 和 `--once`。
